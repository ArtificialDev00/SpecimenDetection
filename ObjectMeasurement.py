...
videocapture=cv2.VideoCapture("sample2.mp4")
...
def rescale_frame(frame, percent=25):  # make the video windows a bit smaller
    width = int(frame.shape[1] * percent/ 20)
    height = int(frame.shape[0] * percent/ 20)
    dim = (width, height)
...
while(showLive):
    ...
    if areas:
        ...
        print("No contours found or areas calculated.")
